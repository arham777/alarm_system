...
def analyze_alarm_flooding(file_path, header_row=8, count_threshold=10, time_window_minutes=10):
    """
    Analyzes a CSV file for alarm flooding events using an improved sliding window.

    Improvements:
    - Continuous sliding without clearing to handle overlaps.
    - Merges overlapping/adjacent candidate windows into maximal clusters.
    - Computes actual rate based on real duration (not fixed 10 min).
    - Flood count reflects peak density in the merged span.
    ...
    try:
        ...
        for source in unique_sources:
            ...
            if len(source_alarms) < count_threshold:
                continue  # Skip if too few events

            # Use a deque to maintain a sliding window of events within the time window
            window = deque()
            candidates = []  # Collect qualifying windows for merging
            ...
            for idx, alarm in source_alarms.iterrows():
                ...
                # Remove any alarms from the left of the window that are outside the time window
                while window and window[-1] - window[0] > timedelta(minutes=time_window_minutes):
                    window.popleft()

                # If the number of alarms in the window meets the threshold, record as candidate
            ...
            # Merge overlapping/adjacent candidates to form maximal clusters
            epsilon = timedelta(seconds=1)  # Tolerance for adjacent
            merged = []
            current = candidates[0]
            for next_cand in candidates[1:]:
                if next_cand['start'] <= current['end'] + epsilon:
                    # Merge: extend span, take max count
                    current['end'] = max(current['end'], next_cand['end'])
                    current['count'] = max(current['count'], next_cand['count'])
                else:
                    merged.append(current)
                    current = next_cand
            merged.append(current)
            
            # For each merged cluster, compute duration and rate, then record
            for m in merged:
                duration_min = (m['end'] - m['start']).total_seconds() / 60.0
                rate_per_min = m['count'] / duration_min if duration_min > 0 else 0
                # Record the event details
                flooding_events.append({
                    'File Path': file_path,  # Add the file path to the report
                    'Alarm Source': source,
                    'Flood Count': m['count'],
                    'Start Time': m['start'].strftime('%Y-%m-%d %H:%M:%S.%f'),
                    'End Time': m['end'].strftime('%Y-%m-%d %H:%M:%S.%f'),
                    'Location': m['location'],
                    'Setpoint Value': m['value'],
                    'Duration Min': round(duration_min, 2),
                    'Rate Per Min': round(rate_per_min, 2)
                })
